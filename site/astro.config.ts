...
// PR previews are built with SITE_URL=https://pr-<n>.thomasjbarlow.com so
// canonical links and the sitemap point at the preview, not production.
const site = process.env.SITE_URL || siteConfig.url;

// https://astro.build/config
export default defineConfig({
	site,
	// Matches the nginx config: every page is a directory with an index.html.
	...
	integrations: [
		...
		mdx(),
		robotsTxt({
			policy: [{ userAgent: "*", disallow: ["/admin", "/oauth"] }],
		}),
		webmanifest({
