...
;DATA					    ;Syrian Emiration Rate Statistics for Migrnts Lost at Sea from  01/10/2015 to 24/08/2016
   ...
   giData ftgen 0, 0, -31, -2,112,255,661,844,1388,1875,2860,3618,4555,4862,5047,5262,5493.25,5958.5,6157.75,7435,7820.25,7944.5,8268.75,9132,9625.25,10252.5,10491.75,10857,11410,11623,12078,12778,14100,14619,14954,15025,15025
...
;iDatLo2 = 0  ;259
	...
	iDatReadRate = p3
	iDatLen ftlen giData         
	kDatRead line 0, iDatReadRate, iDatLen 	                
	kDatIn tablei kDatRead,    giData
...
kDens1 =  kdensA1 + (kdensB1 - kdensA1)  *kMorph1
kDur1    =    kdurA1 + (kdurB1 - kdurA1)         *kMorph1

	ktHi1     =  ktHiA1 + (ktHiB1- ktHiA1)   *kMorph1
	ktLo1    =  ktLoA1 + (ktLoB1 - ktLoA1)  *kMorph1


kTrans1 random ktLo1,ktHi1
...
kDens2 =  kdensA2 + (kdensB2 - kdensA2)  *kMorph2
kDur2    =    kdurA2 + (kdurB2 - kdurA2)         *kMorph2

	ktHi2     =  ktHiA2 + (ktHiB2- ktHiA2)   *kMorph2
	ktLo2    =  ktLoA2 + (ktLoB2 - ktLoA2)  *kMorph2


kTrans2 random ktLo2,ktHi2


;ares          fog     xamp,     xdens,         xtrans,             aspd,          koct,      kband,      kris,      kdur,       kdec,      iolaps,      ifna,   ifnb,     itotdur   [, iphs] [, itmode] [, iskip]
aWaveR  fog       .5,          kDens2,       kTrans2,                a5,        ky1,          .5,       .001,        kDur2,       .001,         3000,          1,     2,           p3,      0,            0	                           
aWaveL  fog       .5,          kDens1,      kTrans1,                a5,             ky,           .1,       .001,        kDur1,       .001,         3000,          1,     2,           p3,      0,            0	                           
 
  ; aWaveL = aWaveL1 + aWaveL2
  ; aWaveR = aWaveR1 + aWaveR2

    
    aWaveL lowpass2 aWaveL, 10000*ky1, .5
    aWaveR lowpass2 aWaveR, 10000*ky1, .5
...
<CsScore>
f 1 0 16777216 1 "Waves1.wav" 0 0 0
