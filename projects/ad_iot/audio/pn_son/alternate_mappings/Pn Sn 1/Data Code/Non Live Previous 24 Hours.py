...
###BinLogic 
    ...
    if binstart <= tstamp <= binend :    #if current time falls within the bin time
        ...
        if tstamp >= lts: # if data ends before bin ends, dump counters to arrays
            ...
            msr= msr + [msgcount]       #Arrays to capture aggregates from bins
            rsr= rsr + [rssicount]
...
scan = 0   #initialise scanning variable
fin = len(evr) #end the loop
...
while scan != fin:
    ...
    msg.append([evntScn, msgScn,rsiScn])          # Append the data to the root 
    ...
    time.sleep(.5)   #Inserts the 500 milisecond rest to create the 120 bpm over a minute long file
