...
It also uses weather data from the OpenWeather Maps API including rainfall, windpseed and temperature levels. Theoretical frameworks from Embodied Cognitive Science and music theory were once again used to guide the initial design of the system.


Traditional approaches to mapping data to sound (parameter mapping sonification) were employed alongside approaches that involve machine learning.

The system represents traffic data and weather data with distinct sound objects. Data is then mapped to modulate specific parameters of those objects.
...
The system is comprised of four component layers: Data Acquisition & Processing, the Generative Engine,  Sound Synthesis Engine, and Post Processing. The output consists of three data-driven sound objects presented in sequence. The first object represents weather data the second represents traffic data and the third represents the number off available bikes at city bike stands. When attended to in sequence, the sonification is intended to give an overall sense of the state of the city in terms of threse three caegories. Fitch and Kramer discuss the concept of “piggy backing” as a sonification mapping strategy in which multiple streams of data are mapped to different parameters of a single sound. The sound objects generated by this system use the same approach. 
