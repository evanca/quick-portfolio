...
#injson = requests.get("https://ns1.pervasivenation.com/trace/json?tz=0&event=!GW|PXY&from=10d&num=99999&pltspan=w", auth=('pervasive.nation', '98ojnv12-dws')).content
#injson = requests.get("https://ns1.pervasivenation.com/trace/json?tz=-0&event=DL&cmd=JoinAccept&from=20%2F04%2F2016&to=20%2F04%2F2017&num=99999&pltspan=w", auth=('pervasive.nation', '98ojnv12-dws')).content


injson = requests.get("https://ns1.pervasivenation.com/trace/json?tz=-0&event=!GW|PXY&from=10d&num=9999999999&pltspan=w", auth=('pervasive.nation', '98ojnv12-dws')).content
...
numOfDays = 10                       # Number of Days
then = now - (dayms * numOfDays)         # ## X days ago in unix

fts =  ts.iloc[0]         # ## First Time Stamp in Data
lts = ts.tail(1).iloc[0]  # ts.iloc[len(ts)-1] ## Last Time Stamp in Data

if fts < then:    
    binstart = fts           #use  the earliest possible timestamnp.
elif fts > then:
    binstart = then
           
binsize = 7200000          # Want 120 2 hour bins 720,0000ms = 12 mins
...
evr =[]  #Lists to capture aggregates from bins
msr  =[] 
rsr  =[]
dvr = []
gtr = []          
...
###BinLogic 
    ...
    if binstart <= tstamp <= binend :    #if current time falls within the bin time
        ...
        if dvAddr != "" and dvAddr != du.iloc[y-1]:   #if current device is not 0 and is not same as last one count it
            dvcount = dvcount +1
        if gate != "" and gate != gw.iloc[y-1]:
            gtcount = gtcount+1
        ...
        if tstamp >= lts: # if data ends before bin ends, dump counters to arrays
            ...
            eventcount = 0 #RESET EVENT BIN COUNTER
            joincnt = 0 #RESET MSG BIN COUNTER
            rssicount = 0 #RESET RSSI BIN COUNTER
            dvcount = 0 #RESET DEVICE BIN COUNTER
            gtcount = 0 #RESET GATEWAY BIN COUNTER
